<template>
  <div>
    <div class="container-fluid">
      ...
      <h2 class="text-center">Danh sách các toà nhà</h2>
      ...
      <div class="mt-4">
        ...
        <div>
          <DataTable :columns="columns" :data="fbuildings" class="table table-striped" width="100%">
...
const getNextId = () => {
  ...
  return Math.max(...buildings.value.map(b => b.id)) + 1;
};

const addBuilding = () => {
  newBuilding.value.id = getNextId();
  buildings.value.push({ ...newBuilding.value });
  saveBuildings();
  resetForm();
  Swal.fire({
    position: 'top-end',
...
<!-- không liên quan tới api , nhập và hiển thị, thêm sửa xoá -->
